  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (request.method === 'POST' && url.pathname === '/mcp') {
        ...
        return Response.json({
          ...
          result: {
            tools: [{
              ...
              description: 'Analyzes orderbook depth and imbalance for trading decisions',
              inputSchema: {
                type: 'object',
                properties: {
                  coins: { 
      ...
      if (body.method === 'tools/call') {
        const sessionId = env.ORDERBOOK_ANALYZER_MCP.newUniqueId();
        const stub = env.ORDERBOOK_ANALYZER_MCP.get(sessionId);
    ...
    console.log(`Orderbook Analyzer MCP - sessionId: ${sessionIdStr} with id: ${id}`);
